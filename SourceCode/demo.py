...
from AIPlanner.compriser_controled import Interpret
...
all_topics = [
    ...
    ("test/weather", 1),
    ("bpe/data/location/actuator/luminance", 1),
    ("bpe/data/location/actuator/temperature", 1),
]
...
def callback(client, userdata, msg):
    global state, brightness_set, temperature_set
    ...
    if "weather" in str(msg.topic):
        state[4] = str(msg.payload)[2:-1]

    if "actuator" in str(msg.topic):
        if "luminance" in str(msg.topic):
            brightness_set = float(str(msg.payload)[2:-1])
            # if brightness_set >= 255:
            #     brightness_set = 255
            # if brightness_set <= 0:
            #     brightness_set = 0

        if "temperature" in str(msg.topic):
            temperature_set = float(str(msg.payload)[2:-1])
...
def increase_temperature(publisher: Publisher):
    ...
    logger.debug(
        f"increase_temperature : Increasing temperature by {temperature_delta}"
    )
...
def decrease_temperature(publisher: Publisher):
    global temperature_set
    logger.debug(
        f"decrease_temperature : Decreasing temperature by {temperature_delta}"
    )
    publisher.publisherVariable = temperature_set - temperature_delta
    temperature_set = temperature_set - temperature_delta
...
def increase_brightness(publisher: Publisher):
    ...
    logger.debug(f"increase_brightness : Increasing brightness by {brightness_delta}")
    if int(brightness_set + brightness_delta) >= 255:
        publisher.publisherVariable = brightness_set + brightness_delta
        brightness_set = brightness_set + brightness_delta
    else:
        publisher.publisherVariable = 255
        brightness_set = 255


def decrease_brigthness(publisher: Publisher):
    global brightness_set
    logger.debug(f"decrease_brigthness : Decreasing brightness by {brightness_delta}")
    if not (int(brightness_set - brightness_delta) <= 0):
        publisher.publisherVariable = brightness_set - brightness_delta
        brightness_set = brightness_set - brightness_delta
    else:
        publisher.publisherVariable = 0
        brightness_set = 0
...
def main(
    ...
    replacement_dict, optimal = InterP.give_dict(
        temperature_val=state[0],
        luminance_val=state[1],
        window_status=state[3],
        weather_string=state[4],
    )
