...
class Interpret:
    ...
    *** IF WINDOW CLOSED : window_status = False ***
    ...
    def __evaluate_weather(self):
        gloomy_word_list = ["clouds", "rain", "snow"]
        if str(self.owm_client.get_status()) in gloomy_word_list:
    ...
    def give_dict(
        self, temperature_val: float, luminance_val: float, window_status: bool
    ) -> dict:
